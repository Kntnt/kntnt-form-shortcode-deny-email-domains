...
trait Settings {
    ...
    public function options_page() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( __( 'Unauthorized use.', 'kntnt-form-shortcode-mautic' ) );
        }
    ...
    private function render_settings_page() {
        ...
        if ( $unsatisfied_dependencies = Plugin::unsatisfied_dependencies() ) {
            $message = sprintf( _n( 'This plugin must be installed and active: %s', 'These plugins must be installed and active: %s', count( $unsatisfied_dependencies ), 'kntnt-form-shortcode-mautic' ), join( ', ', $unsatisfied_dependencies ) );
    ...
    private function notify_error( $field ) {
        if ( isset( $field['validate-error-message'] ) ) {
            ...
            $message = sprintf( __( '<strong>ERROR:</strong> Invalid data in the field <em>%s</em>.', 'kntnt-form-shortcode-mautic' ), $field['label'] );
        }
        else {
            $message = __( '<strong>ERROR:</strong> Please review the settings and try again.', 'kntnt-form-shortcode-mautic' );
        }
        $this->notify_admin( $message, 'error' );
    }

    private function notify_success() {
        $message = __( 'Successfully saved settings.', 'kntnt-form-shortcode-mautic' );
